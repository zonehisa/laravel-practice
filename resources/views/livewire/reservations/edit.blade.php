...
mount(function (Reservation $reservation) {
    ...
    if ($reservation->status === ReservationStatus::Cancelled) {
        abort(403, 'キャンセル済みの予約は編集できません。');
    }

    $this->reservation = $reservation;
    $this->start_datetime = $reservation->start_datetime->format('Y-m-d\TH:i');
    $this->end_datetime = $reservation->end_datetime->format('Y-m-d\TH:i');
    $this->program_name = $reservation->program_name;
    ...
    $this->source = $reservation->source->value;
...
<div class="max-w-4xl mx-auto p-6">
    <div class="mb-6">
        <h1 class="text-3xl font-bold">予約編集</h1>
        <a href="{{ route('reservations.show', $reservation) }}" class="text-blue-600 hover:text-blue-800 mt-2 inline-block">
            ← 詳細に戻る
        </a>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="source" wire:model="source"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @foreach(ReservationSource::cases() as $sourceOption)
